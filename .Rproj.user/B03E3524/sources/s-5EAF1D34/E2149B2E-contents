...
write('output_dir: "docs"', file="_bookdown.yml",append=TRUE)
...
# Come manual tweaks in index.Rmd 

# get epub

download.file("https://cran.r-project.org/doc/manuals/r-release/R-ints.epub", destfile = "r-ints.epub")
unzip(zipfile = "r-ints.epub")
file.remove(c("toc.ncx","titlepage.xhtml", "stylesheet.css"))

# Rename file and keep a track of file change
rename_file <- function(name){
  new_file <- gsub("(R-ints)_split_([0-9]*)", "\\2-\\1", name)
  new_file <- gsub("^0", "", new_file)
  file.rename(from = name, to = new_file)
  return(data.frame(orig = name, new = new_file))
}

file_names_change <- purrr::map_df(list.files(pattern = "ints_split"), rename_file)
...
purrr::walk(list.files(pattern = "R-ints.html"), html_converter)
...
replace_in_file("06-R-ints.Rmd", "./R-exts.html#Creating-R-packages", "http://colinfay.me/writing-r-extensions/creating-r-packages.html")
